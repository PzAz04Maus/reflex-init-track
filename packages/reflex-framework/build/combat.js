...
export const ENCUMBRANCE_BASE_INITIATIVE = {
    overloaded: 5,
    heavy: 7,
    moderate: 9,
    light: 12,
...
export const WAIT_TICK_COST_BY_OODA = [
    { min: 15, cost: 1 },
    { min: 13, cost: 2 },
    { min: 10, cost: 3 },
    { min: 7, cost: 4 },
...
export const TACTICAL_MOVEMENT_EFFECTS = {
    sprint: [
...
export function getBaseInitiative(encumbrance) {
    return ENCUMBRANCE_BASE_INITIATIVE[encumbrance];
}
export function computeMargin(roll, target) {
    return target - roll;
}
export function computeOodaAdjustedInit(baseInit, roll, target, pressBonus = 0) {
    const margin = computeMargin(roll, target);
    return baseInit + pressBonus + (margin > 0 ? margin * 2 : 0);
}
export function computeInitiative(encumbrance, roll, target, pressed = false) {
...
export function startExchange(participants) {
    const exchangeParticipants = participants.map((participant) => ({
        actorId: participant.actorId,
        ...computeInitiative(participant.encumbrance, participant.roll, participant.ooda, participant.pressed ?? false),
    }));
...
export function getPauseMovementDistance(tacticalMoveDistance) {
    return tacticalMoveDistance * PAUSE_MOVEMENT_MULTIPLIER;
}
export function resolveExchangeContinuation(choices) {
    ...
    return {
        ...
        pressBonusByActorId: Object.fromEntries(normalizedChoices.map((choice) => [choice.actorId, choice.finalChoice === 'press' ? 5 : 0])),
        normalizedChoices,
    };
}
export function resolvePauseContinuation(choices, pausesSinceLastExchange) {
    const outcomes = choices.map((choice) => {
        ...
        return {
            actorId: choice.actorId,
            declaredChoice: choice.choice,
            finalChoice: succeeded ? 'hold' : 'press',
            cufPenalty: pausesSinceLastExchange,
...
export function createCombatAction(key, options = {}) {
...
export function createPendingAction(id, name, cost) {
    return {
        id,
        key: 'pending',
        name,
